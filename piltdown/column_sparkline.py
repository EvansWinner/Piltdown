...
def column_sparkline(data: List[Tnum], bottom:Tnum = 0) -> str:
    ...
    if min(data) >= bottom:
        ...
        raise ValueError("Bottom must be less than or equal to lowest data value")
    scaler = 1 / (max(data) / 8) 
    data = list(map(lambda x: math.floor(x * scaler), data))
    ret = ""
    for i in data:
        ret+=lit.V8THS[i]
    return ret        

print(column_sparkline([3,5,4,3,8,4,34,2,5,34,4,12]))
